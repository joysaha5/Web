...
```
index.html      Home — hero + featured apps
apps.html       Our Digital Creations — Elements, N Dial, N Files, N Calc
team.html       Meet the Makers
store.html      Store — Coming Soon
timeline.html   Studio timeline
contact.html    Say Hello — contact form + direct links
css/style.css   All styling, design tokens, dark/light themes, responsive rules
js/script.js    Theme toggle (persisted via localStorage), mobile menu, starfield canvas
```
...
- **Accent color** — change `--accent` and `--accent-strong`.
...
- **Team photos** — replace the `.avatar-fallback` divs in `team.html` with `<img>` tags pointing at real photos once you have them.
